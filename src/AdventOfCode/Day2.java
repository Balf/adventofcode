    ...
    public static void main(String[] args) {
        ...
        for (String row : rows) {
            ...
            for (int i = 0; i < numberList.size(); i++) {
                ...
                int heighNumber = numberList.get(i);

                for (int j = (i + 1); j < numberList.size(); j++) {
                    if (heighNumber % numberList.get(j) == 0) {
                        //resultList.add(heighNumber / numberList.get(j));

                        difference += (heighNumber / numberList.get(j));
        ...
        System.out.println("Hello, World, the answer is: " + difference);
    }
}
